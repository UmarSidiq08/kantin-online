                ...
                if ($transactionStatus == 'settlement' || ($transactionStatus == 'capture' && $fraudStatus == 'accept')) {
                    try {
                        ...
                        if (!$userId || !$amount) {
                            ...
                            return response()->json(['message' => 'Missing top-up data'], 200);
                        }
            ...
            elseif (str_starts_with($orderId, 'ORDER-')) {
                if ($transactionStatus == 'settlement' || ($transactionStatus == 'capture' && $fraudStatus == 'accept')) {
                    try {
                        ...
                        DB::beginTransaction();
                        //oke jadi saya kan punya sebuah website laravel 10 tentang  kantin di mana user bisa melakukan pesanan. nah saya ingin menambahkan fitur stok dimana
                        //stok akan berkurang saat pesanan diterima namun stok akan kembali saat pesanan di tolak dan user tidak bisa pesan saat stok habis.
