    ...
    <div class="bg-white shadow-lg rounded-xl border border-gray-100 overflow-hidden">
        ...
        <div class="bg-gradient-to-r from-green-50 to-emerald-50 px-4 sm:px-8 py-6 border-b border-gray-100">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                ...
                <button
                    class="bg-gradient-to-r from-green-500 to-green-600 text-white px-6 py-3 rounded-lg hover:from-green-600 hover:to-green-700 transition-all duration-200 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 font-semibold flex items-center justify-center w-full sm:w-auto"
        ...
        <div class="p-4 sm:p-8">
            ...
            <div id="empty-state" class="hidden">
                <div class="text-center py-12">
                    ...
                    <button
                        class="bg-gradient-to-r from-green-500 to-green-600 text-white px-6 py-3 rounded-lg hover:from-green-600 hover:to-green-700 transition-all duration-200 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 font-semibold flex items-center mx-auto"
                        data-bs-toggle="modal" data-bs-target="#modal_add">
                        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path>
...
@section('script')
    <script>
        ...
        // Fix: tambah parameter stok dan image, perbaiki logika preview gambar
        const onEdit = (id, name, description, category, price, stok, image) => {
            ...
            $('#modal_edit input[name="stok"]').val(stok);

            // Reset input file supaya tidak ada sisa upload sebelumnya
            ...
            if (image && image !== '') {
                ...
                // Pakai CSS langsung, bukan class hidden (campuran Tailwind + Bootstrap)
                $('#edit_image_preview_container').css('display', 'block');
            } else {
                $('#edit_existing_image').val('');
                $('#edit_image_preview_container').css('display', 'none');
            }
        };
        ...
        $(function() {
            ...
            $('#modal_add').on('show.bs.modal', function() {
                $('#form_add button[type="submit"]').prop('disabled', false).html(
                    '<i class="fas fa-save me-2"></i>Simpan');
            });
            ...
            $('#modal_edit').on('hidden.bs.modal', function() {
                $('#form_edit button[type="submit"]').prop('disabled', false).html(
                    '<i class="fas fa-save me-2"></i>Perbarui');
                // Reset preview gambar saat modal ditutup
                $('#edit_image_preview_container').css('display', 'none');
                $('#edit_image_input').val('');
            });
        });
    ...
    <style>
        #edit_image_preview_container {
            display: none;
            /* Default tersembunyi, ditampilkan via JS */
        }
