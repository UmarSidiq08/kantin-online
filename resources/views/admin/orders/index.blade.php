                ...
                @if ($orders->where('status', \App\Constant::ORDER_STATUS['PENDING'])->count() > 1)
                    <div class="flex space-x-3">
                        <button type="button" id="bulk-accept-all"
                            ...
                            <span>Terima Semua Pesanan
                                ({{ $orders->where('status', \App\Constant::ORDER_STATUS['PENDING'])->count() }})</span>
                        </button>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" id="orders-container">
            @forelse ($orders as $order)
                @php
                    ...
                    $isUnpaidCash = $order->payment_method === 'cash' && $order->payment_status === 'unpaid';
                    $bgClass = $isPaid ? 'bg-green-50' : ($isUnpaidCash ? 'bg-yellow-50' : 'bg-white');
                    $borderClass = $isPaid
                        ? 'border-green-400'
                        : ($isUnpaidCash
                            ? 'border-yellow-400'
                            : 'border-gray-200');
                @endphp
                <div class="{{ $bgClass }} rounded-lg shadow-sm border-2 {{ $borderClass }} hover:shadow-md transition-all duration-200 flex flex-col h-full"
    ...
    <script>
        $(document).ready(function() {
            ...
            function showAlert(message, type = 'success') {
                const toastClass = type === 'success' ? 'bg-green-500 border border-green-600 text-white' :
            ...
            function removeOrderCard(orderId) {
                $(`[data-order-id="${orderId}"]`).fadeOut(300, function() {
                    $(this).remove();
                    if ($('#orders-container [data-order-id]').length === 0) location.reload();
                });
            }
            ...
            $('#bulk-accept-all').on('click', function() {
                ...
                if (!confirm(`Yakin ingin menerima semua ${totalOrders} pesanan sekaligus?`)) return;
                    ...
                    setTimeout(() => {
                        $.ajax({
                            ...
                            success: function(response) {
                                ...
                                $(`[data-order-id="${orderId}"]`).fadeOut(300,
                                    function() {
                                        $(this).remove();
                                    });
                                if (processedCount === totalOrders) {
                                    setTimeout(() => {
                                        $('#bulk-progress').addClass(
                                            'hidden');
                                        showAlert(
                                ...
                                if (processedCount === totalOrders) {
                                    setTimeout(() => {
                                        $('#bulk-progress').addClass(
                                            'hidden');
                                        showAlert(
                                            `Selesai memproses. ${successCount} berhasil${errorCount>0?`, ${errorCount} gagal`:''}`,
                                            errorCount > 0 ?
                                            'error' : 'success');
                                        if (successCount > 0) location
                                            .reload();
                                    }, 500);
                                }
                            }
            ...
            $('#bulk-reject-all').on('click', function() {
                ...
                if (!confirm(`Yakin ingin menolak semua ${totalOrders} pesanan sekaligus?`)) return;
                $('#bulk-progress').removeClass('hidden');
                updateProgress(0, totalOrders);
                $('#bulk-accept-all, #bulk-reject-all').prop('disabled', true).addClass('opacity-50');
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        updateProgress(totalOrders, totalOrders);
                        setTimeout(() => {
                            ...
                            if (response.success) {
                                ...
                                showAlert(response.message || 'Terjadi kesalahan',
                                    'error');
                                $('#bulk-accept-all, #bulk-reject-all').prop('disabled',
                                    false).removeClass('opacity-50');
                            }
                        }, 500);
                    },
                    error: function(xhr) {
                        clearInterval(progressInterval);
                        $('#bulk-progress').addClass('hidden');
                        $('#bulk-accept-all, #bulk-reject-all').prop('disabled', false)
